...
class ItemsController extends LfmController
{
    ...
    public function doMove()
    {
        ...
        foreach ($items as $item) {
            ...
            $new_path = $this->lfm->setName($item)->dir($target)->path();
            if ($is_directory) {
                event(new FolderIsMoving($old_path, $new_path));
            } else {
                event(new FileIsMoving($old_path, $new_path));
            }
    ...
    public function duplicate()
    {
        ...
        return view('laravel-filemanager::duplicate')
            ->with([
                'root_folders' => array_map(function ($type) use ($folder_types) {
                    $path = $this->lfm->dir($this->helper->getRootFolder($type));
    ...
    public function doduplicate()
    {
        ...
        foreach ($items as $item) {
            ...
            if ($old_file->hasThumb()) {
                ...
                $this->lfm->setName($item)->thumb()->duplicate($new_file);
            }
            $new_file = $this->lfm->setName($item)->dir($target);
            $this->lfm->setName($item)->duplicate($new_file);
            if ($is_directory) {
                //event(new FolderWasMoving($old_path, $new_file->path()));
            } else {
                //event(new FileWasMoving($old_path, $new_file->path()));
